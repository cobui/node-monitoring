...
requests.mark(1, { route: "/api/users", method: "GET" });
...
| ----------- | ----------- | ---------------------- | ------------------------------------------------- |
| `Counter`   | counter     | `mark(delta?, tags?)`  | Events (requests, errors, cache hits)             |
...
hits.mark(); // +1
hits.mark(5); // +5
hits.mark({ route: "/api" }); // +1 with tags (shorthand)
...
transporter:
  type: influx
  version: 2
  key: influx # default: "influx". Namespaces sharing the same key share one queue
...
| `namespace`     | The namespace (e.g. `app`)            | All metrics in one place; URI becomes a `metric` tag |
...
Each warning fires at most once per sensor instance.
